...
fn test_cosh() {
    ...
    let a = FixedTrait::ONE();
    assert_precise(cosh(a), 6627480862, 'invalid neg one', Option::None(())); // 1.5430806347841253

    let a = FixedTrait::new(TWO, true);
    assert_precise(cosh(a), 16158507454, 'invalid neg two', Option::None(())); // 3.762195691016423
}
...
fn test_sinh() {
    ...
    assert_precise(sinh(a), -5047450693, 'invalid neg one', Option::None(())); // -1.1752011936029418

    let a = FixedTrait::new(TWO, true);
    assert_precise(sinh(a), -15577246839, 'invalid neg two', Option::None(())); // -3.6268604077773023
}
...
fn test_tanh() {
    ...
    let a = FixedTrait::new(ONE, true);
    assert_precise(tanh(a), -3271021993, 'invalid neg one', Option::None(())); // -0.7615941559446443

    let a = FixedTrait::new(TWO, true);
    assert_precise(tanh(a), -4140466929, 'invalid neg two', Option::None(())); // 0.9640275800745076
}
...
fn test_atanh() {
    ...
    assert_precise(atanh(a), 2359251925, 'invalid half', Option::None(())); // 0.5493061443340548

    let a = FixedTrait::ZERO();
    assert(atanh(a).into() == 0, 'invalid zero');

    let a = FixedTrait::new(HALF, true); // 0.5
    assert_precise(atanh(a), -2359251925, 'invalid neg half', Option::None(())); // 0.5493061443340548
